...
import { Menu, X, ChevronRight, Users, CheckSquare, Sparkles, Zap, Bot, Phone, MessageSquare, BarChart3, Shield, Star, ArrowRight, Play, Globe, Megaphone, Database, Target, CalendarCheck, Mail, Send } from 'lucide-react';
        ...
        <div className="text-center max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-16 animate-fadeIn" style={{animationDelay: '0.6s'}}>
            {[
              ...
              { number: '200k', label: t.hero.stats.conversations },
          ...
          <div className="flex flex-col items-center gap-4 animate-fadeIn" style={{animationDelay: '0.8s'}}>
            ...
            <p className="text-sm text-gray-400">
              {t.hero.socialProof} <span className="text-white font-semibold"> {t.hero.companies}</span> {t.hero.trust}
            </p>
          </div>
        </div>
      </div>
...
function FeaturesSection({ t }: { t: any }) {
  const icons = [Megaphone, Database, Target, CalendarCheck, Phone, Mail];
  const colors = [
...
// Solutions Section
function SolutionsSection({ t, locale }: { t: any; locale: string }) {
  const [selectedSolution, setSelectedSolution] = useState(0);
  const icons = [Megaphone, Database, CalendarCheck, Send];
  const colors = [
    "from-blue-500 to-cyan-500",
    "from-purple-500 to-pink-500",
    "from-green-500 to-emerald-500",
          ...
          {t.solutions.items.map((solution: any, index: number) => {
            ...
            return (
              ...
              >
                ...
                <a href={`/${locale}/contact`} className="flex items-center gap-2 text-blue-400 font-semibold group-hover:gap-4 transition-all">
...
// CRM Banner Section
function CrmBannerSection({ t, locale }: { t: any; locale: string }) {
  return (
    <section className="relative py-24 bg-gradient-to-r from-blue-900/30 via-purple-900/30 to-blue-900/30 border-y border-white/10">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="flex-1 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full mb-6">
              <Database className="w-4 h-4 text-blue-400" />
              <span className="text-sm text-blue-300 font-medium">{t.crm.badge}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 leading-tight">
              {t.crm.title}
            </h2>
            <p className="text-lg text-gray-400 mb-4 max-w-xl">
              {t.crm.subtitle}
            </p>
          ...
          <div className="flex-shrink-0 w-full lg:w-auto">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-4 max-w-sm mx-auto">
              {t.crm.perks.map((perk: string, i: number) => (
              ...
              <a
                href={`/${locale}/contact`}
                className="mt-6 w-full py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-full font-semibold text-sm flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-blue-500/40 transition-all duration-300"
              >
                {t.crm.cta}
                <ArrowRight className="w-4 h-4" />
              </a>
...
export default function App() {
  ...
  return (
    <div className="bg-black min-h-screen">
      ...
      <IntroSection t={t} />
      <FeaturesSection t={t} />
      <SolutionsSection t={t} locale={locale} />
      <CrmBannerSection t={t} locale={locale} />
